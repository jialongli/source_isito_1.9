		...
		case networking.ServiceEntry_STATIC:
			...
			for _, endpoint := range serviceEntry.Endpoints {
				...
				if strings.HasPrefix(addr, UnixAddressPrefix) {
					...
					//注意,如果serviceentry下的port字段中如果没有定义  serviceentry/endpoints/port对象的话,就会报错,也就是说
					//如果serviceentry下port定义了[{"name":"http","number":"28880"},{"name":"dubbo","number":"28881"}]那么下面的endpoints只能出现上面这两种,不然报错
