...
func (sink *Sink) handleResponse(resources *mcp.Resources) *mcp.RequestResources {
	...
	if resources.Collection == "istio/networking/v1alpha3/virtualservices" || resources.Collection == "istio/networking/v1alpha3/destinationrules" {
		fmt.Print("===come=====")
	}
...
//[mcp]istio<-->mcpServer  创建请求pr,顺序的发送每一种类型的pr.  都发送完毕后,再处理响应
