...
func (c *controller) Apply(change *sink.Change) error {
	...
	for _, obj := range change.Objects {
        ...
        //这里存储到namespace--config.name的map中,如果resource中有两个同名的配置,那么会覆盖.
        /**
        例如 virtualservice name:test1
         	virtualservice name:text1
        	所以同名的配置,只能有一个.不然会覆盖
         */
	...
	//!!!!!这里把当前数据存储到configStore中,之后推送啥的,都要根据这个里面的存储进行计算!!!!!!!!!
