...
//===初始化configController===
//1.如果是mcpServer,初始化mcpConfigController
//2.如果是内存
// initConfigController creates the config controller in the pilotConfig.
func (s *Server) initConfigController(args *PilotArgs) error {
	meshConfig := s.environment.Mesh()
	if len(meshConfig.ConfigSources) > 0 {
		log.Info("===初始化配置中心,meshConfig.ConfigSources>0,因此使用mcpServer")
		// Using MCP for config.
		if err := s.initMCPConfigController(args); err != nil {
			return err
		}
	} else if args.Config.FileDir != "" {
		log.Info("===初始化配置中心,args.Config.FileDir != 空")
	...
	log.Info("===初始化配置中心,打印下是否有k8s注册中心 hasKubeRegistry" + strconv.FormatBool(hasKubeRegistry(args.Service.Registries)))
...
func (s *Server) initMCPConfigController(args *PilotArgs) (err error) {
	...
	s.addStartFunc(func(stop <-chan struct{}) error {
		...
		//[startFunc]1.初始化mcpClient,调用run.可能有多个mcpServer.
		for i := range clients {
			client := clients[i]
			wg.Add(1)
			go func() {
