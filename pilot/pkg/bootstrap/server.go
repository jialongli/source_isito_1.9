...
func NewServer(args *PilotArgs) (*Server, error) {
	...
	//1.初始化kubeclient,如果是k8s作为注册中心的话.我们使用mcpServer
	...
	//2.初始化meshConfig.这个启动配置--meshConfig=/Users/lijialong/Documents/config/meshConfig
	//并启动一个监听器,监听文件的变化.
	...
	//4.初始化配置控制器.我们使用了mcpServer,因此会初始化mcp相关
	if err := s.initConfigController(args); err != nil {
		return nil, fmt.Errorf("config controller: %v", err)
	}

	//5.初始化注册中心控制器,此控制器能够从底层服务发现机制中获取Istio抽象服务
	...
	//6.初始化发现服务，发现服务依赖于前面创建的配置控制器、服务控制器，对xDS的支持有发现服务提供
	...
	//=====此处是 ValidatingWebhookConfiguration的处理逻辑,在添加isito cr时,会来istio做校验,我们不使用istio crd而是使用mcp.因此可以注销掉
...
func (s *Server) initKubeClient(args *PilotArgs) error {
	...
	log.Info("=====初始化kubeclient========kubeConfig:::"+args.Config.KubeConfig)
	var err error
	...
	//}else {
	//	log.Info("=====不使用k8s注册中心,因此不初始化kubeclient========")
	//}
...
func (s *Server) initDiscoveryService(args *PilotArgs) error {
	...
	//1.注册一些事件变更处理器,包括service/endpoints/配置
	if err := s.initEventHandlers(); err != nil {
		return err
	}
	//2.[===核心==]envoyXdsServer启动,主要做三件事
	//2.1.handleUpdates 死循环,监听配置变更,并将变更推送给 sidecar.只是把内容push到队列
	//2.2periodicRefreshMetrics 定时更新metrics
	//2.3.sendPushes 这里也是死循环从队列中获取要推送的事件.和1共同完成推送.
...
func (s *Server) initEventHandlers() error {
	...
	//3.注册配置变更的处理函数,例如virtualservice变更,destinationrule变更.注意serviceentry不需要注册.
